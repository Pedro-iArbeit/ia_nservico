...
# importa utilitários e caminhos do helpers (mantém o teu helpers.py)
from .helpers import (
...
# ✅ CORRECT: passa a CLASSE + kwargs (não instancies CORSMiddleware diretamente)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],          # troca por https://iarbeit.eu se quiseres restringir
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# expõe os ficheiros gerados (XML/ZIP)
...
async def append_nota(
    Data: str = Form(...),
    HoraInicio: str = Form(...),  # UI: "HH:MM"
    HoraFim: str = Form(...),     # UI: "HH:MM"
    Tempo: float = Form(...),     # horas decimais (min/60)
...
def make_docs(rows):
    # agrupa por Cliente + NIF + Data (um documento por cliente/dia)
    groups = {}
    for r in rows:
